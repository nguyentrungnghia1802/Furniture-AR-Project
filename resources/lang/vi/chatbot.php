<?php

return [
    'greeting' => "Xin chào! Tôi là trợ lý AI của Luna Shop. Tôi có thể giúp bạn với:\n"
        ."🌸 Tìm kiếm & gợi ý sản phẩm\n"
        ."📝 Theo dõi & trạng thái đơn hàng\n"
        ."📋 Danh mục & bộ sưu tập sản phẩm\n"
        ."📰 Tin tức & cập nhật blog mới nhất\n"
        ."💰 Thông tin giá & thanh toán\n"
        ."🚚 Chi tiết vận chuyển & giao hàng\n"
        ."🏪 Thông tin cửa hàng & liên hệ\n"
        ."❓ Câu hỏi thường gặp\n\n"
        .'Tôi có thể giúp gì cho bạn hôm nay?',

    'help' => "🤖 Hướng dẫn sử dụng chatbot Luna Shop:\n\n"
        ."🔍 Tìm sản phẩm:\n"
        ."• 'tìm nội thất'\n"
        ."• 'cho tôi xem quà tặng'\n"
        ."• 'sản phẩm phổ biến'\n"
        ."• 'gợi ý sản phẩm'\n\n"
        ."📂 Duyệt danh mục:\n"
        ."• 'danh mục sản phẩm'\n"
        ."• 'hiển thị danh mục'\n"
        ."• 'bạn bán gì'\n\n"
        ."📦 Kiểm tra đơn hàng:\n"
        ."• 'đơn hàng của tôi'\n"
        ."• 'trạng thái đơn hàng'\n"
        ."• 'theo dõi đơn hàng'\n\n"
        ."📰 Tin tức & cập nhật:\n"
        ."• 'tin tức mới nhất'\n"
        ."• 'bài viết blog'\n"
        ."• 'có gì mới'\n\n"
        ."🏪 Thông tin cửa hàng:\n"
        ."• 'thông tin cửa hàng'\n"
        ."• 'chi tiết liên hệ'\n"
        ."• 'giờ mở cửa'\n"
        ."• 'địa chỉ'\n\n"
        ."💰 Giá & thanh toán:\n"
        ."• 'khoảng giá'\n"
        ."• 'phương thức thanh toán'\n"
        ."• 'chi phí vận chuyển'\n\n"
        .'Hãy hỏi tôi bất cứ điều gì bằng ngôn ngữ tự nhiên!',

    'store_info' => "🏪 Thông tin cửa hàng Luna Shop:\n\n"
        ."📍 Địa chỉ: 123 Luna Street, Sơn Tây, Hà Nội\n"
        ."📞 Điện thoại: 0353295709\n"
        ."📧 Email: support@hanayashop.com\n"
        ."🕒 Giờ làm việc: 8:00 AM - 10:00 PM (Thứ 2 - Chủ nhật)\n\n"
        ."🚚 Giao hàng: Toàn quốc\n"
        ."💳 Thanh toán: Tiền mặt, chuyển khoản, thẻ tín dụng/ghi nợ\n"
        ."🎁 Dịch vụ đặc biệt: Gói quà, sắp xếp theo yêu cầu\n\n"
        .'Liên hệ ngay để được tư vấn cá nhân hóa!',

    'status' => [
        'pending' => 'Chờ xử lý - Đơn hàng của bạn đang được xử lý',
        'processing' => 'Đang xử lý - Chúng tôi đang chuẩn bị đơn hàng của bạn',
        'shipped' => 'Đã giao - Đơn hàng của bạn đang trên đường',
        'completed' => 'Hoàn thành - Đơn hàng đã được giao thành công',
        'cancelled' => 'Đã hủy - Đơn hàng đã bị hủy',
    ],

    'price_info' => "Về giá sản phẩm của chúng tôi:\n\n"
        ."🛋️ Nội thất: Từ $50 - $5000\n"
        ."🪑 Ghế: Từ $50 - $1500\n"
        ."🪟 Bàn & Trang trí: Từ $30 - $2000\n"
        ."🎀 Đơn đặt hàng tùy chỉnh: Từ $100 - $5000\n\n"
        ."Giá có thể thay đổi theo chất liệu, kích thước, tay nghề và tuỳ chỉnh.\n"
        ."💡 Miễn phí vận chuyển cho đơn hàng trên $100!\n"
        ."🎊 Giảm giá cho đơn hàng số lượng lớn!\n\n"
        .'Xem giá chi tiết tại: /products',

    'shipping_info' => "🚚 Thông tin vận chuyển & giao hàng:\n\n"
        ."📦 Miễn phí vận chuyển cho đơn hàng trên $100\n"
        ."🚚 Giao hàng tiêu chuẩn: 3-5 ngày làm việc\n"
        ."⚡ Giao hàng nhanh: 1-2 ngày làm việc (+$15)\n"
        ."🏃 Giao trong ngày: Có tại Hà Nội (+$25)\n\n"
        ."📍 Khu vực giao hàng: Toàn quốc\n"
        ."📋 Theo dõi: Có theo dõi đơn hàng thời gian thực\n"
        ."📦 Đóng gói: Thân thiện môi trường, an toàn\n"
        ."🎁 Quà tặng: Gói quà & thiệp miễn phí\n\n"
        .'Có thắc mắc về giao hàng? Liên hệ: 0353295709',

    'payment_info' => "💳 Phương thức & tuỳ chọn thanh toán:\n\n"
        ."💰 Thanh toán khi nhận hàng (COD)\n"
        ."🏦 Chuyển khoản ngân hàng (xác nhận ngay)\n"
        ."💳 Thẻ tín dụng/ghi nợ (Visa, Mastercard)\n"
        ."📱 Ví điện tử (Paypal)\n"
        ."💎 Có trả góp\n\n"
        ."🔒 Thanh toán bảo mật\n"
        ."🎊 Ưu đãi: Giảm 5% khi chuyển khoản ngân hàng\n"
        ."💝 Điểm thưởng cho mỗi lần mua\n\n"
        .'Cần hỗ trợ thanh toán? Chúng tôi luôn sẵn sàng!',

    'default' => "Tôi hiểu bạn cần hỗ trợ! Đây là những gì tôi có thể giúp:\n\n"
        ."🔍 **Tìm kiếm sản phẩm**: 'tìm nội thất', 'cho tôi xem quà tặng'\n"
        ."📂 **Danh mục**: 'bạn có danh mục gì'\n"
        ."📦 **Đơn hàng**: 'kiểm tra đơn hàng', 'theo dõi đơn hàng'\n"
        ."📰 **Tin tức**: 'cập nhật mới nhất', 'bài viết blog'\n"
        ."🏪 **Thông tin cửa hàng**: 'chi tiết liên hệ', 'giờ mở cửa'\n"
        ."💰 **Giá**: 'khoảng giá', 'phương thức thanh toán'\n"
        ."🚚 **Vận chuyển**: 'tùy chọn giao hàng', 'chi phí vận chuyển'\n\n"
        ."💡 **Mẹo nhanh**: Hãy hỏi tự nhiên, ví dụ 'Bạn gợi ý đồ nội thất nào cho phòng khách?' hoặc 'Chi phí vận chuyển là bao nhiêu?'\n\n"
        ."Gõ 'help' để xem hướng dẫn chi tiết hoặc hỏi tôi bất cứ điều gì!",

    // Error handling
    'error' => '🤖 Xin lỗi, tôi gặp sự cố kỹ thuật. Vui lòng thử lại sau hoặc liên hệ bộ phận hỗ trợ qua số :phone để được trợ giúp ngay.',

    // Product search responses
    'no_products_found' => "🔍 **Không tìm thấy sản phẩm phù hợp**\n\n"
        ."Bạn có thể quan tâm đến:\n"
        ."🛋️ Bộ sưu tập nội thất: Sofa, bàn, tủ\n"
        ."🪑 Ghế: Ghế và ghế đẩu\n"
        ."🎁 Quà lưu niệm: Món quà ý nghĩa, độc đáo\n\n"
        ."🔗 :products_url\n\n"
        ."💡 **Mẹo tìm kiếm:**\n"
        ."• Thử các từ khóa rộng hơn như 'hoa' hoặc 'quà tặng'\n"
        ."• Tìm theo dịp: 'sinh nhật', 'đám cưới'\n"
        ."• Duyệt danh mục để lấy cảm hứng\n\n"
        .'📞 **Cần hỗ trợ?** Gọi: :phone',

    'products_search_results' => '🛋️ **Sản phẩm phù hợp với tìm kiếm của bạn:**',

    'browse_more_products' => "✨ **Xem thêm sản phẩm:**\n"
        ."🔗 :products_url\n\n"
        ."💡 **Mẹo mua sắm:**\n"
        ."• Kiểm tra tình trạng còn hàng trước khi đặt\n"
        ."• Đọc mô tả sản phẩm để biết kích thước\n"
        ."• Liên hệ để được tư vấn cá nhân\n"
        .'• Hotline: :phone',

    // Category responses
    'no_categories_found' => "📂 **Danh mục sản phẩm**\n\n"
        ."Chúng tôi đang cập nhật danh mục sản phẩm.\n"
        ."Vui lòng quay lại sau hoặc xem tất cả sản phẩm:\n\n"
        ."🔗 :products_url\n\n"
        .'📞 **Cần trợ giúp?** :phone',

    'product_categories' => '📂 **Danh mục sản phẩm tại Luna Shop:**',

    'popular_categories' => "🎯 **Danh mục phổ biến:**\n"
        ."🪑 Ghế - Thoải mái và phong cách\n"
        ."🪟 Bàn - Tiện dụng và thanh lịch\n"
        ."🎁 Trang trí - Món quà ý nghĩa, đáng nhớ\n\n"
        .'💝 **Mẹo:** Chọn theo dịp đặc biệt như sinh nhật, cưới hỏi, kỷ niệm...',

    // Order responses
    'login_required' => "🔐 **Vui lòng đăng nhập để kiểm tra đơn hàng**\n\n"
        ."📱 **Đăng nhập để truy cập:**\n"
        ."• Lịch sử & theo dõi đơn hàng\n"
        ."• Cập nhật trạng thái giao hàng\n"
        ."• Hóa đơn điện tử\n"
        ."• Đặt lại sản phẩm yêu thích\n\n"
        ."🔗 :login_url\n\n"
        ."❓ **Cần hỗ trợ?** Liên hệ:\n"
        ."📞 :phone\n"
        .'📧 :email',

    'no_orders_found' => "📦 **Không tìm thấy đơn hàng nào**\n\n"
        ."🛒 **Bắt đầu mua sắm:**\n"
        ."🔗 :products_url\n\n"
        ."🎁 **Ưu đãi đặc biệt:**\n"
        ."• Miễn phí vận chuyển cho đơn trên $100\n"
        ."• Giảm 10% cho khách hàng lần đầu\n"
        ."• Gói quà miễn phí\n\n"
        .'📞 **Cần hỏi đáp?** :phone',

    'recent_orders' => '📦 **Đơn hàng gần đây của bạn:**',

    'order_support' => "🔍 **Cần hỗ trợ thêm?**\n"
        ."📞 Hotline: :phone\n"
        ."📧 Email: :email\n"
        .'⏰ Thời gian hỗ trợ: 8:00 - 22:00 hàng ngày',

    'order_technical_error' => "📦 **Thông tin đơn hàng tạm thời không khả dụng**\n\n"
        ."Chúng tôi đang gặp sự cố kỹ thuật khi truy cập thông tin đơn hàng.\n\n"
        ."📞 **Cần hỗ trợ đơn hàng ngay, vui lòng liên hệ:**\n"
        ."• Điện thoại: :phone\n"
        ."• Email: :email\n"
        ."• Thời gian hỗ trợ: 8:00 - 22:00 hàng ngày\n\n"
        .'Xin lỗi vì sự bất tiện này và cảm ơn bạn đã thông cảm! 🙏',

    // News responses
    'no_news_found' => "📰 **Tin tức & cập nhật**\n\n"
        ."Hiện chưa có tin tức mới.\n"
        ."Vui lòng quay lại sau để cập nhật!\n\n"
        ."🌸 **Theo dõi chúng tôi để nhận tin mới:**\n"
        ."• Ra mắt sản phẩm\n"
        ."• Khuyến mãi đặc biệt\n"
        ."• Mẹo & hướng dẫn chăm sóc\n\n"
        .'📞 **Liên hệ:** :phone',

    'latest_news' => '📰 **Tin tức & bài viết mới nhất:**',

    'hot_topics' => "🌸 **Chủ đề nổi bật:**\n"
        ."• Cách chọn nội thất cho từng phòng\n"
        ."• Mẹo bảo quản và chăm sóc đồ nội thất\n"
        ."• Ý tưởng trang trí & bố trí nội thất\n"
        ."• Xu hướng quà tặng 2025\n\n"
        .'💡 **Truy cập website để đọc thêm nhiều bài viết thú vị!**',

    // Popular products responses
    'no_popular_products' => "🔥 **Sản phẩm phổ biến**\n\n"
        ."Chúng tôi đang cập nhật danh sách bán chạy.\n"
        ."Xem tất cả sản phẩm để tìm món quà tuyệt vời:\n\n"
        ."🔗 :products_url\n\n"
        .'📞 **Cần tư vấn?** :phone',

    'top_bestselling' => '🔥 **Sản phẩm bán chạy nhất:**',

    'why_customers_love' => "⭐ **Vì sao khách hàng yêu thích:**\n"
        ."• Chất lượng cao, bền đẹp\n"
        ."• Giá trị xứng đáng\n"
        ."• Đóng gói sang trọng\n"
        ."• Dịch vụ khách hàng tận tâm\n\n"
        .'🛒 **Đặt hàng ngay để nhận ưu đãi!**',

    // Gift suggestions
    'gift_suggestions' => "🎁 **Gợi ý quà tặng hoàn hảo từ Luna Shop:**\n\n"
        ."💝 **Danh mục quà tặng phổ biến:**\n"
        ."🛋️ Bộ ghế sofa nhỏ\n"
        ."🎀 Hộp quà sang trọng, đóng gói đẹp\n"
        ."💐 Sắp xếp theo yêu cầu cho dịp đặc biệt\n"
        ."🌸 Thiệp nhắn cá nhân miễn phí\n\n"
        ."🔗 :products_url\n\n"
        ."💡 **Dịp tặng quà:** Sinh nhật, kỷ niệm, Valentine, Ngày của mẹ, cưới hỏi, tốt nghiệp\n\n"
        .'📞 **Cần tư vấn cá nhân?** Gọi: :phone',

    'perfect_gifts' => '🎁 **Gợi ý quà tặng hoàn hảo cho bạn:**',

    'why_gifts_special' => "🌟 **Vì sao quà tặng của chúng tôi đặc biệt:**\n"
        ."• Thủ công tỉ mỉ, chăm chút từng chi tiết\n"
        ."• Bền đẹp, lưu giữ kỷ niệm\n"
        ."• Đóng gói sang trọng miễn phí\n"
        ."• Có thiệp nhắn cá nhân\n\n"
        .'💝 **Phù hợp mọi dịp đặc biệt!**',

    // Availability responses
    'availability_info' => '📦 **Thông tin tình trạng sản phẩm:**',
    'limited_stock' => 'Sản phẩm sắp hết',
    'only_left' => 'Chỉ còn :count sản phẩm!',
    'out_of_stock' => 'Hiện đã hết hàng',
    'restock_soon' => 'Sắp có hàng lại',

    'stock_updates' => "✅ **Cập nhật tồn kho:**\n"
        ."• Sản phẩm bán chạy được nhập hàng hàng tuần\n"
        ."• Hàng mới về mỗi tháng\n"
        ."• Đăng ký nhận thông báo khi có hàng\n\n"
        .'📞 **Cần kiểm tra tồn kho cụ thể:** :phone',

    // --- Assistant Info ---
    'name' => 'Trợ lý Luna Shop',
    'status_online' => 'Đang trực tuyến',
    'greeting_hanaya' => 'Xin chào! Tôi là trợ lý AI của Luna Shop. Tôi có thể giúp bạn với:',
    'how_can_i_assist' => 'Tôi có thể giúp gì cho bạn hôm nay? 🌸',

    // --- Main Features ---
    'find_products' => '🔍 Tìm kiếm & gợi ý sản phẩm',
    'track_orders' => '📦 Theo dõi đơn hàng',
    'pricing_info' => '💰 Giá & thanh toán',
    'faq' => '❓ Hỏi đáp & hỗ trợ khách hàng',
    'soap_flowers' => '🪑 Ghế',

    // --- Quick Action Buttons ---
    'track_orders_button' => '📦 Đơn hàng của tôi',
    'store_info_button' => '🏪 Thông tin cửa hàng',
    'latest_news_button' => '📰 Tin tức',
    'popular_button' => '⭐ Phổ biến',
    'help_button' => '❓ Trợ giúp',
];
