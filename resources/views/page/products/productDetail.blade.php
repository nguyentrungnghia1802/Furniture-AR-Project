            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="flex items-center space-x-4 text-sm text-gray-500 mb-4">
                            <span class="flex items-center">
                                <i class="fas fa-eye mr-1"></i>{{ $product->view_count ?? 0 }} {{ __('product.views') }}
                            </span>
                            <span class="flex items-center">
                                <i class="fas fa-box mr-1"></i>{{ $product->stock_quantity }} {{ __('product.in_stock') }}
                            </span>
                            <span class="flex items-center">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <label for="quantity" class="block mb-3 font-medium text-gray-700">{{ __('product.choose_quantity') }}</label>
                            ...
                            @if ($product->stock_quantity > 0)
                                ...
                                <form action="{{ route('cart.buyNow', $product->id) }}" method="POST" class="w-full">
                                    ...
                                    <button type="submit" class="w-full bg-orange-600 hover:bg-orange-700 text-white font-semibold py-3 px-6 rounded-lg shadow-lg transition-colors duration-300 flex items-center justify-center">
                                        <i class="fas fa-bolt mr-2"></i>{{ __('product.buy_now') }}
                                    </button>
                                </form>
                            @else
                                <button 
                        ...
                        @else
                            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                                <div class="flex items-center justify-center mb-2">
                                    ...
                                    <span class="text-yellow-700 font-medium">{{ __('product.sign_in_required') }}</span>
                                </div>
                                <p class="text-sm text-yellow-600 mb-3">{{ __('product.sign_in_message') }}</p>
                                <div class="space-y-2">
                                    <a href="{{ route('login') }}"
                                        ...
                                        <i class="fas fa-sign-in-alt mr-2"></i>{{ __('product.sign_in') }}
                                    </a>
                                    <a href="{{ route('register') }}"
                                        ...
                                        <i class="fas fa-user-plus mr-2"></i>{{ __('product.create_account') }}
                                    </a>
                                </div>
                            </div>
                        @endauth
        ...
        <div class="mt-12 sm:mt-16">
            <div class="bg-white rounded-lg shadow-lg p-6 sm:p-8">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl sm:text-2xl font-bold text-gray-900">{{ __('product.customer_reviews') }}</h3>
                    <div class="flex items-center space-x-2">
                        <x-star-rating :rating="$averageRating" size="lg" show-text />
                        <span class="text-sm text-gray-600">({{ $totalReviews }} {{ __('product.reviews') }})</span>
                    </div>
                </div>
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <h4 class="text-lg font-medium text-gray-900 mb-2">{{ __('product.no_reviews_yet') }}</h4>
                        <p class="text-gray-600">{{ __('product.be_first_review') }}</p>
                    </div>
                @endif
            </div>
        </div>
