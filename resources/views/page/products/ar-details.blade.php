...
@section('title', $product->title . ' - AR View')
...
<div class="container mx-auto px-4 py-8">
    ...
    <nav class="mb-8">
        <ol class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <li class="flex items-center"><span class="mx-2">/</span></li>
            {{-- Nếu có route cho category thì giữ, nếu không thì xóa dòng này hoặc thay bằng '#' --}}
            <li><a href="#" class="hover:text-blue-600">{{ $product->category->name }}</a></li>
            <li class="flex items-center"><span class="mx-2">/</span></li>
            <li class="text-gray-900 font-medium">{{ $product->title }} (AR View)</li>
        </ol>
    </nav>
                ...
                <div class="related-product-card bg-white border border-gray-200">
                    <div class="aspect-square bg-gray-100 relative overflow-hidden">
                        @if($related->images->count() > 0)
                            <img src="{{ $related->images->first()->url }}" alt="{{ $related->title }}" class="w-full h-full object-cover">
                        @endif
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-gray-600 text-sm mb-2">{{ $related->category->name }}</p>
                        <div class="flex items-center justify-between">
                            <span class="text-lg font-bold text-blue-600">${{ number_format($related->price, 2) }}</span>
                            <a href="{{ route('products.ar', $related->slug) }}" class="bg-purple-100 text-purple-700 px-3 py-1 rounded-lg text-sm font-medium hover:bg-purple-200 transition-colors">
                                View AR
                            </a>
                        </div>
                    </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg max-w-md w-full p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button onclick="ArHelpers.hideArInstructions()" class="text-gray-400 hover:text-gray-600">
            ...
            <button onclick="ArHelpers.hideArInstructions()" class="w-full mt-6 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition-colors">
                Got it!
            </button>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const message = ArHelpers.checkCompatibility();
